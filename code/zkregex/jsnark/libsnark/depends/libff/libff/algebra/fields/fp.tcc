...
//ADDED By CorrAuthor ---------------------
// quick initliaze some static members that are needed for
// generating R1CS's prime filed operation (mainly mul and add)
// inv, and Rsquared (which are needed for mul_reduce), Rcubed.
// NOTE: we did not set s, t, , euler and others because
// we did not see them being used in generating r1cs constraints

template<mp_size_t n, const bigint<n>& modulus>
void Fp_model<n,modulus>::quick_init_for_gen_r1cs(){
	//1. compute inv inv = modulus^(-1) * (W-1) mod W
	// w: 64, W = 2^w, 
	mpz_t mpz_modulus;
	mpz_init(mpz_modulus);
	modulus.to_mpz(mpz_modulus);
	mpz_t two;
	mpz_init(two);
	mpz_set_ui(two, 2);
	int w = 64; //64-bit numbers
	mpz_t W;
	mpz_init(W);
	mpz_pow_ui(W, two, w);
	...
	mpz_t mod_inv;
	mpz_init(mod_inv);
	mpz_powm_ui(mod_inv, mpz_modulus, -1, W);

	mpz_t W_minus_1;
	mpz_init(W_minus_1);
	mpz_sub_ui(W_minus_1, W,  1);

	mpz_t mpz_inv;
	mpz_init(mpz_inv);
	mpz_mul(mpz_inv, mod_inv, W_minus_1);
	mpz_mod(mpz_inv, mpz_inv, W);

	bigint<1> biinv(mpz_inv);
	inv = biinv.data[0]; //coz it's guaranteed to be 64-bit
	

	//2. compute Rsquared and Rcubed
	// r = 2^{s*w} where w is the word width 64, s is the number of
    //   legs (e.g. 4)
    //rsquared is r*r % modulus
	mpz_t mpz_r;
	mpz_init(mpz_r);
	mpz_pow_ui(mpz_r, two, n*w); //here s is the n for n-leg mpz
	mpz_t mpz_r2; 
	mpz_init(mpz_r2);
	mpz_mul(mpz_r2, mpz_r, mpz_r);
	mpz_mod(mpz_r2, mpz_r2, mpz_modulus);
	mpz_t mpz_r3;
	mpz_init(mpz_r3);
	mpz_mul(mpz_r3, mpz_r, mpz_r2);
	mpz_mod(mpz_r3, mpz_r3, mpz_modulus);
	Rsquared = bigint<n>(mpz_r2); 
	Rcubed = bigint<n>(mpz_r3);
...
//Added By CorrAuthor --------------
template<mp_size_t n, const bigint<n>& modulus>
void Fp_model<n,modulus>::print_to(FILE *fp) const
{
    Fp_model<n,modulus> tmp;
    tmp.mont_repr.data[0] = 1;
    tmp.mul_reduce(this->mont_repr);
    tmp.mont_repr.print_to(fp);
}
//Added by CorrAuthor Above ------------
