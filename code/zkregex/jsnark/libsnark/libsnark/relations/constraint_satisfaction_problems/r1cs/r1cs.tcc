...
//#ifdef DEBUG
			...
			printf("constriant %zu unsatisfied\n", c);
...
template<typename FieldT>
void r1cs_constraint_system<FieldT>::save_to(
FILE *fp,
...
{
	fprintf(fp, "field_order: ");
	FieldT::field_char().print_to(fp);
	fprintf(fp, "\n");
	fprintf(fp, "primary_input_size: %d\n", primary_input_size);
	fprintf(fp, "aux_input_size: %d\n", auxiliary_input_size);
	fprintf(fp, "num_constraints: %d\n", num_constraints());
	fprintf(fp, "num_segments: %d\n", num_segments);
	for(int i=0; i<num_segments; i++){
		fprintf(fp, "seg_size: %d\n", seg_size[i]);
	}
	
	//1. dump the variable assignments
	fprintf(fp, "assignments:\n");
    r1cs_variable_assignment<FieldT> full_variable_assignment = primary_input;
    full_variable_assignment.insert(full_variable_assignment.end(), auxiliary_input.begin(), auxiliary_input.end());
	for(int i=0; i<full_variable_assignment.size(); i++){
		fprintf(fp, "%d ", i);
		full_variable_assignment[i].print_to(fp);
		fprintf(fp, "\n");
	}

	//2. dump the constraints
	fprintf(fp, "constraints:\n");
	int i = 0;
    const std::map<size_t, std::string> variable_annotations;
	int bDebug = 0;
    for (const r1cs_constraint<FieldT>& c : constraints)
    {
		c.a.print_to(fp, "A", i);
		c.b.print_to(fp, "B", i);
		c.c.print_to(fp, "C", i);
		if(bDebug){
			printf("constraint %d\n", i);
			printf("A:\n");
			c.a.print_with_assignment(full_variable_assignment, variable_annotations);
			printf("B:\n");
			c.b.print_with_assignment(full_variable_assignment, variable_annotations);
			printf("C:\n");
			c.c.print_with_assignment(full_variable_assignment, variable_annotations);
		}
		i++;
    }
