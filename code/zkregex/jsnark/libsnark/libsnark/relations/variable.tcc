...
//---------- Added By CorrAuthor ------------------
template<typename FieldT>
void linear_combination<FieldT>::print_to(FILE *fp, char *matrix_name, int constraint_id) const
{
    for (auto &lt : terms)
    {
		if(lt.index==0){
			fprintf(fp, "%d %s %d ", constraint_id, matrix_name, -1);
		}else{
			fprintf(fp, "%d %s %d ", constraint_id, matrix_name, lt.index-1);
		}
		//-1 means constants, real variable index starts from 0
		lt.coeff.print_to(fp);
		fprintf(fp, "\n");
    }
}
//------------------- Added by CorrAuthor above
