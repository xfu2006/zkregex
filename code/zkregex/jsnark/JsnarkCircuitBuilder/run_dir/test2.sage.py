
# This file was *autogenerated* from the file test2.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_486662 = Integer(486662); _sage_const_255 = Integer(255); _sage_const_253 = Integer(253); _sage_const_55484635554744707071703875581767296995 = Integer(55484635554744707071703875581767296995); _sage_const_19 = Integer(19)
ec = EllipticCurve(GF(_sage_const_2 **_sage_const_255 -_sage_const_19 ),[_sage_const_0 ,_sage_const_486662 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ])
#p1 = ec.lift_x(4,all)[0]
#print(p1)
#p2 = p1 + p1
#print(p2)
print("2^255-19 is: " + str(_sage_const_2 **_sage_const_255 -_sage_const_19 ));
forder = ec.order();
print("field-order: " + str(forder));
corder = ec.cardinality();
print("curve-order: " + str(corder));
exp_twist = (_sage_const_2 *(_sage_const_2 **_sage_const_255 -_sage_const_19  + _sage_const_1 ) - corder);
p2 = exp_twist/_sage_const_4 ;
print("expected twist order/4: " + str(p2));
p2_2 = _sage_const_2 **_sage_const_253 -_sage_const_55484635554744707071703875581767296995 
print("given in paper: " + str(p2_2));
twist = ec.quadratic_twist().cardinality();
t4 = twist//_sage_const_4 
print("generated twist/4: " + str(twist/_sage_const_4 ));
print("num bits of twist/4: " + str(t4.nbits()));
print("t4 is a prime: " + str(is_prime(t4)));



