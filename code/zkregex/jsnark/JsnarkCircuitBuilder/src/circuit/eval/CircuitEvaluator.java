...
//CHANGED by CorrAuthor 01/22/2023 -> make it Serializable
public class CircuitEvaluator implements Serializable{
...
//Added by CorrAuthor -------------- 12/28/2022
	public BigInteger getWireValueByID(int id){
...
//Added by CorrAuthor -------------- ABOVE
	...
	//--------------- Added by CorrAuthor -------------
	// dirpath - the container dir
	// pname - the platform (prime field name)
	public void writeInputFile(String dirpath, String pname) {
		try {
			LinkedHashMap<Instruction, Instruction> evalSequence = circuitGenerator
					.getEvaluationQueue();

			PrintWriter printWriter = new PrintWriter(
					dirpath + "/" + circuitGenerator.getName() + ".in." + pname);
			for (Instruction e : evalSequence.keySet()) {
				if (e instanceof WireLabelInstruction
						&& (((WireLabelInstruction) e).getType() == LabelType.input || ((WireLabelInstruction) e)
								.getType() == LabelType.nizkinput)) {
